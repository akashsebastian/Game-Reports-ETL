...
class Report():
    ...
    def generate_diff_team_shots_closest_defender(self, team_daily_shots_closest_defender, team_total_shots_closest_defender):
    ...
    def generate_team_reports(self, date):
        ...
        team_diff_shots_closest_defender = self.generate_diff_team_shots_closest_defender(team_daily_shots_closest_defender, team_total_shots_closest_defender)
    ...
    def generate_diff_player_drives(self, player_daily_drives, player_total_drives):
        ...
        final_col_names = [column + '_diff' for column in player_total_drives.drop('player_id', axis = 1).columns]
    ...
    def generate_diff_player_catch_and_shoot(self, player_daily_catch_and_shot, player_total_catch_and_shoot):
        player_total_catch_and_shoot.drop(['player_name', 'team_id', 'team_abbreviation', 'gp', 'w', 'l', 'min'], inplace=True, axis=1)
        final_col_names = [column + '_diff' for column in player_total_catch_and_shoot.drop('player_id', axis = 1).columns]
        ...
        player_daily_catch_and_shot.drop(['player_name', 'team_id', 'team_abbreviation', 'gp', 'w', 'l', 'min', 'date'], inplace=True, axis=1)
        player_daily_catch_and_shot.columns = [str(col) + '_daily' if col != 'player_id' else str(col) for col in player_daily_catch_and_shot.columns]
        merged_df = player_daily_catch_and_shot.merge(player_total_catch_and_shoot,how='inner',on = 'player_id')
        for column in final_col_names:
            column_base = column.rsplit('_', 1)[0]
            merged_df[column] = merged_df.apply(lambda row: row[column_base + '_daily'] - row[column_base + '_total'], axis = 1)
        final_col_names.append('player_id')
    ...
    def generate_diff_player_pull_up_shooting(self, player_daily_pull_up_shooting, player_total_pull_up_shooting):
        ...
        final_col_names = [column + '_diff' for column in player_total_pull_up_shooting.drop('player_id', axis = 1).columns]
