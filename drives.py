...
from utils import connect_to_db, extract_int


class Drives(AbstractNBAConnect):

    def __init__(self, date_from='', date_to='', playerorteam = 'Team'):
        # mm/dd/yyyy
        if not date_from:
            self.table_name = 'drives'
        else:
            if playerorteam == 'Team':
                self.table_name = 'drives_daily'
            else:
                self.table_name = 'drives_player'
    ...
    def upload_to_db(self, rows):
        try:
            ...
            with conn.cursor() as cur:
                # if self.table_name == 'drives':
                #     args = [cur.mogrify('(%s, %s, %s, %s, %s, %s, %s)', x).decode('utf-8') for x in rows]
                #     args_str = ', '.join(args)
                #     cur.execute(
                #         "INSERT INTO drives values " +
                #         args_str +
                #         "ON CONFLICT (team_id) DO UPDATE SET (team_id, gp, drives, fg_m, fg_a, fg_pct, points) = (EXCLUDED.team_id, EXCLUDED.gp, EXCLUDED.drives, EXCLUDED.fg_m, EXCLUDED.fg_a, EXCLUDED.fg_pct, EXCLUDED.points)"
                #     )
                # else:
                #     args = [cur.mogrify('(%s, %s, %s, %s, %s, %s, %s, %s)', x).decode('utf-8') for x in rows]
                #     args_str = ', '.join(args)
                #     cur.execute(
                #         "INSERT INTO drives_daily values " +
                #         args_str +
                #         "ON CONFLICT (team_id, date) DO UPDATE SET (team_id, gp, drives, fg_m, fg_a, fg_pct, points, date) = (EXCLUDED.team_id, EXCLUDED.gp, EXCLUDED.drives, EXCLUDED.fg_m, EXCLUDED.fg_a, EXCLUDED.fg_pct, EXCLUDED.points, EXCLUDED.date)"
                #     )
                args = [cur.mogrify('(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', x).decode('utf-8') for x in rows]
                args_str = ', '.join(args)
                cur.execute(
                    "INSERT INTO player_drives values " +
                    args_str +
    ...
    def parse_data(self, raw_data):
        ...
        for row in raw_data['resultSets'][0]['rowSet']:
            ...
            if self.table_name == 'drives_daily':
